...
def estimate_pdf_size(pages: list, total_pages: int, total_file_size: int) -> int:
    """
    估算PDF页面的大小（字节）
    
    Args:
        pages: 当前页面列表
        total_pages: PDF文件总页数
        total_file_size: PDF文件总大小（字节）
    ...
    estimated_size = int(len(pages) * avg_page_size)

    # 添加10%的误差容忍度，因为PDF压缩等因素可能影响实际大小
...
def split_pdf_by_size(input_path: str, max_size: int, output_template: str, output_dir: str = None, under_action: str = 'skip') -> list[str]:
    ...
    for page_num, page in enumerate(reader.pages, 1):
        # 先检查添加当前页面后是否会超过大小限制
        temp_pages = current_pages + [page]
        estimated_size = estimate_pdf_size(temp_pages, total_pages, file_size)

        # 检查是否需要先保存当前分片（不包含新页面）
        should_save_current = False

        if len(current_pages) > 0 and estimated_size >= max_size:
            # 添加新页面会超过大小限制，先保存当前分片
            should_save_current = True
            logger.debug(f"页面 {page_num}: 添加后将超限 ({format_size(estimated_size)})，先保存当前分片")
